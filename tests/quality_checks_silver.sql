...
-- Check for Invalide Date Orders (Start Date > End Date)
-- Expectation: No Results
SELECT
    *
FROM silver.crm_prd_info
...
SELECT
    *
FROM silver.crm_sales_details
WHERE sls_order_dt < DATE('2010-01-01') OR sls_order_dt > CURRENT_DATE
    OR sls_ship_dt < DATE('2010-01-01') OR sls_ship_dt > CURRENT_DATE
    OR sls_due_dt < DATE('2010-01-01') OR sls_due_dt > CURRENT_DATE + INTERVAL '1 month';
...
-- Data Standardization & Consistencyf
SELECT DISTINCT
    gen
FROM silver.erp_cust_az12;
